...
- ~~Repo isn't under version control yet~~ — done: `git init` + an initial commit (`e8642e4`, 2026-07-25) landed all documented project files (`server.py`, `custom_rules/`, `scripts/`, `tests/`, `playbooks/`, `environment/`, `investigations/`, `.claude/`, docs). `credential_access_rules.json`, `credential_rules`, and `hayabusa_rules_poc.pptx` were deliberately left untracked — they're undocumented scratch/exploration artifacts, not part of the architecture in CLAUDE.md; decide whether to commit, gitignore, or delete them.
...
- **`git init` + initial commit (2026-07-25, `e8642e4`) intentionally excluded `credential_access_rules.json`, `credential_rules`, and `hayabusa_rules_poc.pptx`** from the repo root. All three predate the documented architecture, aren't referenced by `server.py` or any script, and look like leftover exploration output (a keyword dump, freeform notes, a slide deck) rather than authored project content. Left untracked rather than gitignored, since it's still an open question whether they should be committed, moved, or deleted — gitignoring would hide that decision instead of surfacing it.
